...
依赖顺序的构建计划(纯 vibe coding,非时间排期):[planning/v1-build-sequence.md](planning/v1-build-sequence.md)。序列 S0 地基/观测核心 → S1 执行体探针 → S2 选角引擎 → S3 选角屏(英雄)→ S4 护栏 → S5 派发 → S6 评估返工闭环 → S7 执行 console → S8 桌宠 → S9 端到端验收,含四个 GATE。

## 下一步动作

从 S0 开始(地基 + 把 legacy 观测核心挖回)。V1 最该先想透、最值钱的一块是 S2 选角引擎——但它依赖 S0/S1,按序走。
